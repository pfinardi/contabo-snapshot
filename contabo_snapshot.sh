...
# get instances
UUID=$(uuidgen)
instances=($(curl -X GET "https://api.contabo.com/v1/compute/instances" -H 'Content-Type: application/json' -H "Authorization: Bearer ${ACCESS_TOKEN}" -H "x-request-id: ${UUID}" -H "x-trace-id: ${TRACE_ID}" | jq -r '.data[]|{instanceId}[]'))
